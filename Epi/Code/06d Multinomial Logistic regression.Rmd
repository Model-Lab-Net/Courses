...
predict_probs <- predict(mlog_reg_ia, newdata = df4, type = "response")  # Get probabilities
predict_classes <- predict(mlog_reg_ia, newdata = df4, type = "response")  # Get predicted classes

# If levels don't match, ensure they do
levels(predict_classes) <- levels(df4$cat_fbs)

confusion_matrix <- caret::confusionMatrix(data = as.factor(round(df4$predict_classes,0)), reference = as.factor(df4$cat_fbs))


# Extract confusion matrix table for plotting
