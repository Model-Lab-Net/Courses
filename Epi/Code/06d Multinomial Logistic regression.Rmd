...
mlog_reg <- vglm(cat_fbs ~ totchol + weight + hpt, family = multinomial, data = df4, model = TRUE)
summary(mlog_reg)
```

## Model performance measures
```{r model-performance, echo = TRUE}
mlog_reg_perf <- vglm(cat_fbs ~ totchol + weight + hpt, family = multinomial, data = df4, model = TRUE)
# Get -2LL and AIC
...
mlog_reg_ia <- glm(cat_fbs ~ totchol*weight + hpt, data = df4, family = binomial(link = 'logit'))
summary(mlog_reg_ia)
```
