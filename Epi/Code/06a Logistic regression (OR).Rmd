...
tbl_summary(data = df3, by = status)
```
...
r2_val   <- as.numeric(PseudoR2(log_model, which = "Nagelkerke"))
brier <- BrierScore(log_model)
AIC_val  <- as.numeric(AIC(log_model))                       # AIC
chi_sq   <- lrtest(log_model)$Chisq[2]                       # Omnibus χ² (Likelihood Ratio Chi²)
p_val    <- as.numeric(lrtest(log_model)$`Pr(>Chisq)`[2])    # Omnibus χ² p-value (Likelihood Ratio Test)
  p_val <- ifelse(
    p_val < 0.001,
    "<0.001",
    sprintf("= %.3f", p_val)
...
# Perform the Hosmer–Lemeshow test using generalhoslem
hl_test <- logitgof(df3$status, df3$predict.prob, g=10)
hl_test
## PLEASE DO NOT USE THIS - IT IS BAD!!!
