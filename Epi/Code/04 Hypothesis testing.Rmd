...
levene_result <- levene(age_dx_yrs ~ rt, data = df_bc, var.equal = TRUE)
(levene_result)

t_test_result <- t.test(age_dx_yrs ~ rt, data = df_bc, var.equal = FALSE)
t_test_result
```
...
MW_test <- wilcox.test(optimal_systemic_therapy ~ her2_pos, data = df_bc, na.action = na.exclude)
MW_test
```
