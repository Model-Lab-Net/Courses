...
# Perform Mann-Whitney test (paired = FALSE)
MW_test <- wilcox.test(nodal.ratio ~ optimal.systemic.therapy, data = df_bc, na.action = na.exclude, paired = FALSE)
MW_test
```
...
df$grade <- factor(df$grade, levels = c("1", "2", "3"), ordered = TRUE)

# Convert to int variable
df$days.onset.hosp <- as.integer(df$days.onset.hosp)

# rename variables - new_name = old_name
df <- dplyr::rename(df, recurrence = event)

# Check the updated dataset structure
glimpse(df)

# Save as R data file
saveRDS(df, here("EpiData", "02 breastcancer.rds"))
```