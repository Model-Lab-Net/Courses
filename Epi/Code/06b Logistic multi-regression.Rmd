...
### lesson: מודל לוגיסטי 06   
### date  : 30/09/2026
###  
### multivariate נלמד לעשות מודל לוגיסטי מרובה משתנים
###
...
brier <- BrierScore(mlog_model)
chi_sq   <- lrtest(mlog_model)$Chisq[2]                       # Omnibus χ² (Likelihood Ratio Chi²)
p_val    <- as.numeric(lrtest(mlog_model)$`Pr(>Chisq)`[2])    # Omnibus χ² p-value (Likelihood Ratio Test)
  p_val <- ifelse(
    p_val < 0.001,
    "<0.001",
    sprintf("= %.3f", p_val)
...
r2_nagelkerke(log_model)

# Sequential model comparisons
a <- anova(log_model, mlog_model, mlog_model_ia)
...
model_comparison %>%
  as.data.frame() %>%
  dplyr::select(Name, R2_Tjur, RMSE, AICc) %>%
