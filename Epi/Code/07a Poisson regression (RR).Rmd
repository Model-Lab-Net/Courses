...
spr1 <- glm(attack ~ gender, data = df6, family = "poisson")
summary(spr1)

# Do a univrariate Poisson regression - attack vs res_inf
spr2 <- glm(attack ~ res_inf, data = df6, family = "poisson")
summary(spr2)

# Do a univrariate Poisson regression - attack vs ghq12
spr3 <- glm(attack ~ ghq12, data = df6, family = "poisson")
summary(spr3)
```
...
mpr_all = glm(attack ~ ., data = df6, family = "poisson")    # the "." uses all the other variables
summary(mpr_all)

# Do a multivariate Poisson regression - res_inf + ghq12 (not gender)
mpr = glm(attack ~ res_inf + ghq12, data = df6, family = "poisson")
summary(mpr)
```
...
mpr_ia = glm(attack ~ res_inf * ghq12, data = df6, family = "poisson")
summary(mpr_ia)
```

## Model fit and perfoarmance
```{r rmodel-performance, echo = TRUE}
...
pred_values = data.frame(res_inf = rep(c("yes", "no"), each = 4),
                      ghq12 = rep(c(0, 12, 24, 36), 2))
pred_values$attack_pred = round(predict(mpr_ia, new_data, 
                                     type = "response"), 1)
pred_values
```
