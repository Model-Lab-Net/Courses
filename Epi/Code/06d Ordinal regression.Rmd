...
### lesson: מודל אורדינלי 06   
### date  : 30/09/2026
###  
###  נלמד לעשות רגרסיה לוגיסטי כאשר המשתנים הבלתי-תלוי הוא אורדינלי
###
...
df5 <- df5 %>%
  mutate(perforation.ord = factor(perforation, ordered = TRUE))
```

## Perfrom an ordinal linear regression
```{r ord-logisitc-reg, echo = TRUE}
# Ordinal regression model
ord_model <- clm(degree.perforation ~ pulp + hemoglobin + systolic, data = df5, link = "logit", exponentiate = TRUE)
dd <- datadist(df5)   ;   options(datadist = "dd")
ord_model_ <- orm(degree.perforation ~ pulp + hemoglobin + systolic, data = df5,
                  x = TRUE, y = TRUE)     # need this for model diagnostics. sorry.
...
#ord_model <- orm(perforation.ord ~ pulp + hemoglobin + systolic, data = df5)     # need this for model diagnostics. sorry.
