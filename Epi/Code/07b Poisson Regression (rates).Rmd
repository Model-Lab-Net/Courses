...
```{r read-data, echo=FALSE}
## read the data in a file online
df6 %>% mutate(rate = round(case/person_yrs, 4))
glimpse(df6)
```
...
```{r read-data, echo=FALSE}
## read the data in a file online
spr1 <- glm(case ~ cigar_day, data = df6, family = "poisson", offset = log(person_yrs))
summary(spr1)

spr2 <- glm(case ~ smoke_yrs, data = df6, family = "poisson", offset = log(person_yrs))
summary(spr2)
```

## Multiple Poisson regression multivariate analysis for rates
```{r read-data, echo=FALSE}
## read the data in a file online
mpr_all <- glm(case ~ cigar_day + smoke_yrs, data = df6, family = "poisson", offset = log(person_yrs))    # the "." uses all the other variables
summary(mpr_all)
```
...
```{r read-data, echo=FALSE}
## read the data in a file online
mpr_ia <- glm(case ~ cigar_day:smoke_yrs, data = df6, family = "poisson", offset = log(person_yrs))
summary(mpr_ia)
```
