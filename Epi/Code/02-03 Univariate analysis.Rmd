...
# Use summarytools  package
summary_stats <- descr(df1)
summary_stats
```
...
stats <- as.data.frame((summary_stats)) %>%
   cbind(Statistic = rownames(stats), stats)   # need this to keep
...
## Use tbl_summary() to get mean/SD directly
```{r summary-stats, echo = TRUE}
# Create summary statistics for all numerical variables grouped by "outcome"
tbl_summary(data = df1,
...
## Test normality on the raw data (Ho: data are normal dist)
```{r descriptives-old, echo = TRUE}
# Seleect which variable to test
var <- c("age")
# Do Anderson-Darling test for mormality
ad.test((df1[[var]]))
...
```{r boxplot, echo = TRUE}
desc_result <- Desc(df1[[var]], plotit = TRUE)
desc_result
```
...
## Crosstable example (smoking)
```{r chi-square, echo = TRUE}
# Create cross-tabulation/contigency table (with chi-square test)
df_smoking <- data.frame(
  Gender = c("Male", "Male", "Female", "Female"),
  SmokingStatus = c("Smoker", "Non-Smoker", "Smoker", "Non-Smoker"),
  Count = c(40, 60, 20, 80)
 )
df_smoking <- df_smoking %>% 
  uncount(Count) %>%
  mutate(
      SmokingStatus = factor(SmokingStatus, levels = c("Non-Smoker", "Smoker")),
      Gender = factor(Gender)
    )
glimpse(df_smoking)

crosstable <- df_smoking %>%
  finalfit("SmokingStatus", "Gender")
