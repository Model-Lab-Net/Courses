...
ad <- ad.test(df1[[variable]])$p.value      
...
df_smoking %>%
  tbl_summary(
    by = SmokingStatus,                               # Split columns by Smoking Status
    statistic = list(all_categorical() ~ "{p}%"),    # ONLY percentages
    digits = all_categorical() ~ 1
  ) %>%
  add_p(test = all_categorical() ~ "chisq.test") %>% # Add Chi-square
  add_odds_ratio(outcome = SmokingStatus)           # Add Odds Ratio
