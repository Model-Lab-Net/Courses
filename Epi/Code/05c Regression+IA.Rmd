...
### date  : 30/09/2026
###  
###  נלמד מודל לינארי עם אינטראקציה
###
...
pacman::p_load(
  ...
  performance, see,       # get diagnosticsa and model performance
...
# Replave small p-values with <0.001
p_values <- ifelse(
  p_values < 0.001,
  "<0.001",
  sprintf("%.3f", p_values)
...
model_comparison %>%
  as.data.frame() %>%
  dplyr::select(Name, R2_adjusted, RMSE, AICc) %>%
...
model_diag <- mlrIA_model

# Run the diagnostics
sw <- as.numeric(check_normality(model_diag))               # Shapiro-Wilk for normality
bp <- as.numeric(check_heteroscedasticity(model_diag))      # Breusch-Pagan for heteroscedacity
dw <- as.numeric(check_autocorrelation(model_diag))         # Durbin-Watson test for autocorelation
vif <- as.data.frame(check_collinearity(model_diag))           # Mullticolinearity
...
model_diag %>%
  ...
  tab_source_note(
    source_note = html(paste0(
      "Shapiro-Wilk (Normality): ", round(sw, 3), "<br>",
      "Breusch-Pagan (Heteroscedasticity): ", round(bp, 3),"<br>",
      "Durbin-Watson (Autocorrelation): ", round(dw, 3),"<br>"
    )))
...
model_std <- model_parameters(mlrIA, exponentiate = TRUE, standardize = "refit")
